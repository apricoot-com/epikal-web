...
export function BlockPreview({ block }: { block: SiteBlock }) {
    ...
    if (type === 'hero') {
        ...
        return (
            <div className="relative w-full aspect-video bg-gray-100 rounded-md overflow-hidden border flex flex-col justify-center p-4">
    ...
    if (type === 'features') {
        ...
        return (
            <div className="w-full bg-gray-50 border rounded-md p-4 space-y-4">
                <div className="text-center space-y-2">
                    <div className="h-5 bg-gray-800 w-1/3 mx-auto rounded">
                        <span className="text-xs text-secondary-foreground block overflow-hidden h-full">{props.title}</span>
                    </div>
                    <div className="h-2 bg-gray-400 w-1/2 mx-auto rounded"></div>
                </div>
                <div className="grid gap-2" style={{ gridTemplateColumns: `repeat(${cols}, minmax(0, 1fr))` }}>
                    {[1, 2, 3].map((i) => (
                        <div key={i} className="bg-white p-2 rounded shadow-sm flex flex-col items-center space-y-1">
    ...
    if (type === 'content') {
        return (
            <div className={`w-full bg-white border rounded-md p-4 flex flex-col ${props.alignment === 'center' ? 'items-center text-center' : 'items-start text-left'}`}>
                <div className="w-1/2 h-5 bg-gray-800 mb-2 rounded">
                    <span className="text-xs text-white px-1">{props.title || 'Rich Text'}</span>
                </div>
        ...
        return (
            <div className="w-full bg-gray-50 border rounded-md p-4 space-y-4">
                <div className="text-center">
                    <div className="inline-block px-2 text-xs font-bold text-gray-500 uppercase tracking-wider">Services List</div>
                    <div className="h-4 bg-gray-800 w-1/4 mx-auto mt-1 rounded"></div>
                </div>
                <div className="grid grid-cols-3 gap-2">
                    {[1, 2, 3].map(i => (
                        <div key={i} className="aspect-[3/4] bg-white rounded shadow-sm border p-1 flex flex-col">
                            <div className="w-full h-1/2 bg-gray-200 rounded-sm mb-1"></div>
                            <div className="h-3 bg-gray-600 w-3/4 mb-1 rounded"></div>
    ...
    if (type === 'faq') {
        return (
            <div className="w-full bg-white border rounded-md p-4 space-y-3">
                <div className="h-5 bg-gray-800 w-1/4 rounded mb-2">
                    <span className="text-xs text-white px-2">FAQ</span>
                </div>
                {[1, 2].map(i => (
